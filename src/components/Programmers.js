...
export default function Programmers() {
  ...
  const [currentProgrammerId, setCurrentProgrammerId] = useState(""); 
  ...
  const getNameOfFeatured = () => {
    ...
    for(const programmer of listOfProgrammers) {
      if(programmer.id === currentProgrammerId) {
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: currentProgrammerId ? 'orange' : 'royalblue', // 🤔
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfProgrammers.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => {setCurrentProgrammerId(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
